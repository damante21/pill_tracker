import { theme, Breadcrumb, Form, Button, Input, InputNumber, DatePicker, TimePicker } from "antd";
  ...
  return (
    <ILayout>
      ...
      <div
        ...
        <Form labelCol={{ span: 8 }} wrapperCol={{ span: 16 }} form={form} name="control-hooks" onFinish={onFinish} style={{ maxWidth: "70vw" }}>
          ...
          <Form.Item name="intake_quantity" label="Amount of medication" rules={[{ required: true }]}>
            <Input placeholder="Amount of medication taken each time (ie. 2 tablets, 10mL, etc)" />
          </Form.Item>
          <Form.Item name="medication_notes" label="Medication notes" rules={[{ required: false }]}>
            <TextArea placeholder="Reason for medication, take with food, etc" rows={4} />
          </Form.Item>
          <Form.Item name="start_date" label="Start date" rules={[{ required: true }]}>
            <DatePicker />
          </Form.Item>
          <Form.Item name="refill_date" label="Refill date or End Date" rules={[{ required: true }]}>
            <DatePicker />
          </Form.Item>
          <Form.Item name="time_per_day" label="Time per day" rules={[{ required: true }]}>
            <InputNumber min={1} />
          </Form.Item>
          <Form.Item name="time_of_first_med" label="Time of first med" rules={[{ required: true }]}>
            <TimePicker />
          </Form.Item>
          <Form.Item name="number_of_pills" label="Number of pills in bottle" rules={[{ required: true }]}>
            <InputNumber min={1} />
          </Form.Item>
